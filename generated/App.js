...
function App() {
  ...
  const updateMicroTask = (id, timeSpent, timeBudget) => {
    setMicroTasks(microTasks.map(microTask => 
      microTask.id === id ? { ...microTask, timeSpent, timeBudget } : microTask
    ));
  };
  ...
  return (
    <div className="App">
      <div className="section-one">
        ...
        {selectedTask && (
          <>
            <MicroTaskForm createMicroTask={createMicroTask} taskId={selectedTask.id} />
            <MicroTaskList microTasks={microTasks.filter(microTask => microTask.taskId === selectedTask.id)} selectMicroTask={selectMicroTask} />
          </>
        )}
      </div>
      {selectedMicroTask && (
        <Timer microTask={selectedMicroTask} updateMicroTask={updateMicroTask} />
      )}
      <Graph tasks={tasks} />
    </div>
  );
